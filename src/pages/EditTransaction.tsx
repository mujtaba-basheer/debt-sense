...
export default function EditTransaction() {
  ...
  useEffect(() => {
    Promise.all([
      apiFetch("/api/friend").then((r) => r.json()),
      apiFetch(`/api/transaction/${id}`).then((r) => r.json()),
    ])
      .then(([friendsData, txData]: [{ friends: Friend[] }, { transaction: Transaction }]) => {
      ...
      .catch(() => setLoadError("Failed to load transaction"))
    ...
    return (
      <Box sx={{ maxWidth: 600 }}>
        <Button
          ...
          onClick={() => navigate(-1)}
          disableRipple
          sx={{ mb: 3, px: 0, color: COLORS.onSurfaceVariant, fontWeight: 500, "&:hover": { bgcolor: "transparent", color: COLORS.onSurface } }}
        >
          Back
        </Button>
